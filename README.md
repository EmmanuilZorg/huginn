...
This fork includes a `render.yaml` Blueprint that creates a PostgreSQL database,
passes its connection settings to the web service, and runs migrations before
starting Huginn. Apply the Blueprint from this repository instead of entering
placeholder database values such as `your-postgres-host.render.com` manually.
